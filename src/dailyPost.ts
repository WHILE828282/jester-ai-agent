...
export async function runDailyPost(){
  ...
  for(let attempt=1;attempt<=4;attempt++){
    const raw=await generateTweet({ topic, context, successPatterns, failPatterns, recentPosts: recent });
    const tweet=finalizeTweet(normalizeWhitespace(raw));
    const check=validateOutput(tweet);
    if(!check.ok){ log("WARN","Generated tweet rejected",{ attempt, reason: check.reason, tweet }); continue; }
